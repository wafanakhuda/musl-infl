...
router.get("/stats", authMiddleware_1.authenticateToken, (0, express_async_handler_1.default)(async (req, res) => {
    const { user } = req;
    if (!user) {
...
router.get("/earnings", authMiddleware_1.authenticateToken, (0, express_async_handler_1.default)(async (req, res) => {
    const { user } = req;
...
router.get("/platform", (0, express_async_handler_1.default)(async (_req, res) => {
    const totalCreators = await prisma_1.default.user.count({
        where: { user_type: "creator" },
    });
    const totalBrands = await prisma_1.default.user.count({
        where: { user_type: "brand" },
    });
    const totalCampaigns = await prisma_1.default.campaign.count();
    const totalEarnings = await prisma_1.default.transaction.aggregate({
        _sum: { amount: true },
    });
    res.json({
