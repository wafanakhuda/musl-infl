...
export const getDashboardStats = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (userType === "creator") {
    const totalEarnings = dbUser.transactions.reduce(
      (sum: number, txn) => sum + Number(txn.amount),
      0
    )
...
export const getEarningsData = async (req: AuthRequest, res: Response): Promise<void> => {
  const user = req.user
